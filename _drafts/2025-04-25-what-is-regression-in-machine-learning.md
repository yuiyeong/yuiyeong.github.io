...
회귀 모델은 연속형 출력 변수를 예측하는 강력한 도구로, 머신러닝과 통계 분석의 기본이 되는 중요한 개념이다. 선형회귀는 그 중에서도 가장 기본적이고 널리 사용되는 모델로, 데이터 간의 선형 관계를 모델링한다.

상관관계 분석은 변수 간의 관계 강도를 측정하는 방법으로, 회귀 분석과 밀접하게 연관되어 있지만 서로 다른 목적과 해석을 가진다. 상관관계가 있다고 해서 반드시 인과관계가 있는 것은 아니며, 이를 구분하는 것은 데이터 분석에서 매우 중요한 부분이다.

마지막으로, 회귀 분석과 상관관계 분석 모두 특정 가정(선형성, 등분산성, 정규성, 독립성)을 전제로 하며, 이러한 가정이 충족되지 않을 경우 결과의 신뢰성이 감소할 수 있다는 점을 항상 염두에 두어야 한다.
...
- **활용도**: 높음
	- 기초적인 분석과 교육 목적으로 많이 사용됩니다.
	- 가장 단순하고 이해하기 쉬우며, 빠른 분석이 가능합니다.
	- 초기 데이터 탐색과 두 변수 간의 관계 파악에 유용합니다.
...
- **활용도**: 중간~낮음
	- 비선형성이 명확한 특정 도메인에서만 주로 사용됩니다.
	- 단일 변수만으로는 복잡한 현상을 설명하기 어렵습니다.
	- 성장 곡선, 포화 현상 등 비선형 패턴이 명확한 경우에 유용합니다.
...
- **활용도**: 낮음
    - 하나의 입력으로 여러 출력을 예측하는 것은 정보가 부족한 경우가 많습니다.
    - 예측력이 제한적이고 실용성이 떨어집니다.
    - 대부분의 경우 각 출력 변수에 대해 개별 모델을 구축하는 것이 선호됩니다.
...
- **활용도**: 매우 낮음
    - 단순 다변량 선형 회귀의 한계에 비선형성의 복잡성이 더해집니다.
    - 특수한 물리적/화학적 시스템에서 간혹 사용되지만, 대부분의 경우 더 복잡한 모델이 선호됩니다.
    - 현대적 접근법에서는 이런 문제를 다른 방법론으로 해결하는 경우가 많습니다.
...
- **활용도**: 매우 높음 (가장 많이 사용됨)
    - 해석이 직관적이고, 계산이 효율적이며, 대부분의 소프트웨어/라이브러리에서 기본 지원합니다.
    - 다양한 변수를 고려하면서도 단일 목표를 예측하는 상황이 실무에서 가장 일반적입니다.
    - 주택 가격 예측, 판매량 예측, 의학 연구 등 대부분의 예측 작업에 활용됩니다.
...
- **활용도**: 중간~높음
    - 복잡한 현상을 모델링할 때 사용됩니다.
    - 비선형성이 분명한 경우 선형 모델보다 예측력이 우수합니다.
    - 화학 반응 모델링, 날씨 예측, 복잡한 생물학적 시스템 분석 등에서 활용됩니다.
    - 최근에는 머신러닝 기법(랜덤 포레스트, 부스팅 등)으로 대체되는 경향이 있습니다.
...
- **활용도**: 낮음~중간
    - 여러 출력 변수를 동시에 예측하는 것보다 각각 독립적인 모델을 구축하는 것이 종종 더 효과적입니다.
    - 해석이 복잡해지고 각 출력 변수의 최적화가 서로 상충될 수 있습니다.
    - 출력 변수 간 상관관계가 높은 특수한 경우에 유용할 수 있습니다.
...
- **활용도**: 매우 낮음
    - 가장 복잡한 형태의 회귀분석입니다.
    - 계산 비용이 매우 높고, 모델 해석이 어려우며, 과적합 위험이 큽니다.
    - 이러한 복잡한 문제는 요즘에는 딥러닝과 같은 다른 접근법으로 해결하는 경우가 많습니다.
    - 복잡한 시뮬레이션이나 고도의 과학적 연구에서 간혹 사용됩니다.
...
단순 선형 회귀는 Line Fitting 관점에서 이해할 수 있다. 
즉, 직선의 기울기(a)와 y절편(b)을 조절해 데이터(점들)와의 오차가 가장 작아지도록 만드는 과정이다.
...
- 여기서 오차는 목표값과 출력값 간 차이의 제곱으로 정의되는 **MSE(Mean Squared Error)** 손실함수를 사용한다.

$$ Loss_{MSE}: \mathbb{R}, \mathbb{R} \longrightarrow \mathbb{R} $$

$$ y, \hat{y} \longmapsto (y - \hat{y})^2 $$

- 이것은 다시 최적화 문제로 볼 수 있다.
...
> - **수학적 정의**
> $$ $$
> - **시각적 이해**: 함수의 그래프가 아래로 볼록한(밥그릇 모양의) 형태를 가집니다.
> - **중요한 특성**: Convex 함수의 가장 중요한 특성은 **국소 최소값이 전역 최소값**이라는 점 즉, 함수의 값이 감소하다가 증가하기 시작하는 지점이 있다면, 그 지점이 바로 함수의 전체 영역에서의 최소값이다.
{: .prompt-tip}

### 선형 회귀와 Convex 함수의 관계

고정된 X, Y 데이터셋에서 a, b 에 따라 손실함수의 값을 그려보면 convex 함수가 된다. 

MSE 손실함수는 파라미터 a와 b에 대해 convex 함수입니다. 왜냐하면:

1. MSE는 L(a,b)=1n∑i=1n(yi−(axi+b))2L(a, b) = \frac{1}{n}\sum_{i=1}^{n}(y_i - (ax_i + b))^2 L(a,b)=n1​∑i=1n​(yi​−(axi​+b))2 형태로 표현됩니다.
2. 이는 2차 함수(포물선) 형태이며, 항상 아래로 볼록합니다.
3. MSE 함수의 그래프를 3D로 그려보면 밥그릇 모양(위로 볼록)을 가집니다.

이 convex 특성 덕분에:

- 기울기 하강법(gradient descent)과 같은 최적화 알고리즘을 사용하여 쉽게 최적의 파라미터를 찾을 수 있습니다.
- 어디서 시작하든 결국 같은 최적점에 도달합니다(전역 최적해 보장).
- 미분 가능한 지점에서는 기울기(gradient)가 0인 지점이 최소값입니다.

이러한 특성들 때문에 선형 회귀는 수학적으로 매우 잘 정의된 최적화 문제이며, 효율적으로 해결할 수 있습니다.

따라서 선형회귀는 손실함수의 최소값(argmin)을 찾는 컨벡스 최적화(Convex Optimization) 문제로 볼 수 있다.

## 회귀 모델: 최적화의 수치적 해법과 해석적 해법
...
2. **수치적 해법(Numerical Solution)**: 점진적으로 근사적인 해를 찾는 방법
	- 예를 들어, 뉴턴 방법(Newton's Method)은 함수의 접선을 이용해 함수가 0이 되는 지점을 반복적으로 찾아가는 방법

해석적 해법은 빠르고 정확하지만 복잡한 문제에서는 사용하기 어렵다. 

반면 수치적 해법은 근사값을 구하고 시간이 더 걸리지만, 더 다양한 문제에 적용할 수 있다.

## 회귀 모델: 단순 선형 회귀 분석의 해석적 해법
...
여기서 X는 입력 데이터 행렬, y는 출력 벡터, θ는 구하고자 하는 파라미터 벡터이다.

이 공식을 통해 데이터만 입력하면 정확한 최적 파라미터를 바로 계산할 수 있다. 하지만 역행렬을 계산하는 과정이 복잡할 수 있어, 이 부분은 종종 수치적 방법으로 해결한다.


## 회귀 모델: 상관 분석과 상관 계수

- **상관관계(Correlation)**:한 변수가 변화할 때 다른 변수가 함께 변화하는 경향성을 의미한다. 
- **인과관계(Causation)**: 한 변수의 변화가 원인이 되어 다른 변수를 변화시키는 관계를 말한다.

> 인과관계가 있는 변수는 상관관계도 있지만, 상관관계가 있다고 해서 반드시 인과관계가 있는 것은 아니다. 상관관계의 판단은 비교적 쉽지만, 인과관계를 확립하는 것은 훨씬 더 복잡하고 까다롭다.
{: .prompt-tip}

**상관관계 분석(Correlation Analysis)** 은 두 변수 사이의 상관관계를 판단하기 위한 분석 과정이다. 그 결과로 나오는 **피어슨 상관계수(Pearson Correlation Coefficient)** 는 두 변수 사이에 어느 정도로 강한 선형 상관관계가 있는지를 -1에서 1 범위의 숫자로 나타낸다.
...
1. **선형성(Linearity)**: 두 변인 X와 Y의 관계가 직선적이어야 한다.
2. **등분산성(Homoscedasticity)**: X의 값에 관계없이 Y의 분산이 일정해야 한다.
3. **정규성(Normality)**: 각 변인은 모두 정규분포를 따라야 한다.
4. **독립성(Independence)**: 각 샘플들은 모두 독립적이어야 한다.

만약 데이터가 이러한 가정을 충족하지 않는다면, 상관계수를 계산해도 그 의미가 크게 감소한다.

**선형성** 가정은 특히 중요하다. 두 변수 사이에 비선형적 관계가 있더라도, 선형 분석 방법으로는 이를 제대로 포착할 수 없다. 예를 들어, U자형 관계를 가진 데이터는 매우 강한 관계가 있지만, 선형 상관계수는 0에 가까울 수 있다.

**등분산성**은 모든 입력값 범위에서 오차의 분산이 일정해야 함을 의미한다. 이 가정이 충족되지 않는 경우를 **이분산성(Heteroscedasticity)** 이라고 한다.
...
## 회귀 모델: 상관 분석과 선형회귀의 관계

상관계수와 회귀계수는 비슷해 보이지만 서로 다른 개념이다.

- **상관계수**는 상관관계의 강도를 나타낸 것으로, 직선의 기울기가 아니다. 직선의 기울기가 변해도 부호가 바뀌지 않는 한 상관계수의 값은 달라지지 않는다.
- **회귀계수(결정계수)** 는 직선의 기울기 자체를 의미하므로, 데이터의 기울기가 변하면 값도 변한다.
